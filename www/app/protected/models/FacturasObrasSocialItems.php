...
class FacturasObrasSocialItems extends CActiveRecord
{
	...
	public function agregar($idObraSocial,$idFactura,$refactura)
	{
	...
	{
		$criteria=new CDbCriteria;
		$criteria->with=array('facturaProfesional','facturaObraSocial');
		$criteria->compare('facturaProfesional.idProfesional',$idProfesional,false);
		$criteria->compare('YEAR(facturaObraSocial.fecha)',$ano,false);
		if($mes)$criteria->compare('MONTH(facturaObraSocial.fecha)',$mes,false);
		$criteria->order='t.id desc';
		$criteria->select='t.*,(facturaProfesional.importe) as importe';
		if($agrupa){
			$criteria->group="MONTH(facturaObraSocial.fecha)";
			$criteria->select='t.*,SUM(facturaProfesional.importe) as importe';
		}
		return self::model()->findAll($criteria);
	}
