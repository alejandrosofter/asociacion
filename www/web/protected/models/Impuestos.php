...
class Impuestos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('esRetencion', 'numerical', 'integerOnly'=>true),
			array('porcentaje', 'numerical'),
			array('nombreImpuesto', 'length', 'max'=>255),
			array('descripcion', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('buscar,id, nombreImpuesto, porcentaje, descripcion, esRetencion', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'pagosImpuestoses' => array(self::HAS_MANY, 'PagosImpuestos', 'idImpuesto'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('esRetencion',$this->buscar,'OR');
